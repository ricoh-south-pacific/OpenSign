...
const eSigncontact = 'esign@ricohsouthpacific.com';
...
async function sendNotifyMail(doc, signUser, mailProvider, publicUrl) {
  try {
    ...
    const logo =
      "<img src='https://esign.ricohsouthpacific.com/rsp-email-logo.png' height='50' style='padding:20px'/>";
    ...
    if (remainingSign > 1 && doc?.NotifyOnSignatures) {
      ...
      const body =
        ...
        `<div>${logo}</div><div style='padding:2px;font-family:system-ui;background-color:#D7063B'><p style='font-size:20px;font-weight:400;color:white;padding-left:20px'>Document signed by ${signerName}</p>` +
...
async function sendCompletedMail(obj) {
  ...
  let body =
    "<html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /></head><body><div style='background-color:#f5f5f5;padding:20px'><div style='background-color:white'>" +
    `<div>${logo}</div><div style='padding:2px;font-family:system-ui;background-color:#D7063B'><p style='font-size:20px;font-weight:400;color:white;padding-left:20px'>Document signed successfully</p></div><div>` +
